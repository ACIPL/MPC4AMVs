...
% Second scenario: Formation tracking considering the collision
% avoidance, i.e., the collision avoidance cost term is involved in the
% overall objective function
% 2021/07/01 @ ELW 133, UVic
...
% Weighting Matrices
Qi = diag([1e3 1e3 1e2 1e2 1e2 1e2]);
Ri = diag([1e-3 1e-3 1e-2]);
Qfi = diag([1e3 1e3 1e2 1e2 1e2 1e2]);
weights = {Qi,Ri,Qfi};
...
for i=1:1:Tstep
    ...
    [u1, X1_b] = dlmpc1.calc_control_avoidance(P1, ref_trajectory, X1_neighbors, X1_0, u1_0, dt, t1, d1r);
    ...
    [u2, X2_b] = dlmpc2.calc_control_avoidance(P2, ref_trajectory, X2_neighbors, X2_0, u2_0, dt, t1, d2r);
    ...
    [u3, X3_b] = dlmpc3.calc_control_avoidance(P3, ref_trajectory, X3_neighbors, X3_0, u3_0, dt, t1, d3r);
...
figure(2)
subplot(3,1,1)
plot(TIme(1:Tstep),Ua1(1,1:Tstep));
hold on;
plot(TIme(1:Tstep),U1(1,1:Tstep),'LineWidth', 2);
hold on;
plot(TIme(1:Tstep),Ua2(1,1:Tstep));
hold on;
plot(TIme(1:Tstep),U2(1,1:Tstep),'LineWidth', 2);
hold on;
plot(TIme(1:Tstep),Ua3(1,1:Tstep));
hold on;
...
subplot(3,1,2)
plot(TIme(1:Tstep),Ua1(2,1:Tstep));
hold on;
plot(TIme(1:Tstep),U1(2,1:Tstep),'LineWidth', 2);
hold on;
plot(TIme(1:Tstep),Ua2(2,1:Tstep));
hold on;
plot(TIme(1:Tstep),U2(2,1:Tstep),'LineWidth', 2);
hold on;
plot(TIme(1:Tstep),Ua3(2,1:Tstep));
hold on;
