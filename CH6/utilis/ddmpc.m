function [ tmeasure, xmeasure, u, xhat] = ddmpc(runningcosts, terminalcosts, ...
              constraints, terminalconstraints, ...
              linearconstraints, system, ...
                N, T, tmeasure, xmeasure, u0, xhat11, xhat22,VVV, ...
              varargin)
...
%         u = [ u; u_new(:,1)];
        ...
        %[t_intermediate, x_intermediate1, X] = plotTrajectories(dynamic, system, T, t0, x0, u, ...
%                           atol_ode, rtol_ode, type);
        xhat(1,:) = xmeasure;
        for k=1:N
            xhat(k+1,:) = dynamic(system, T, tmeasure, xhat(k,:), u_new(:,k), ...
...
function [u, V, exitflag, output] = solveOptimalControlProblem ...
    ...
    linearconstraints, system, N, t0, x0, u0, T,xhat11, xhat22, VVV,...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)
    ...
    [u, V, exitflag, output] = fmincon(@(u) costfunction(runningcosts, ...
        ...
        u, atol_ode_sim, rtol_ode_sim, type, xhat11, xhat22), u0, A, b, Aeq, beq, lb, ...
        ub, @(u) nonlinearconstraints(constraints, terminalconstraints, ...
        system, N, T, t0, x0, u, ...
        atol_ode_sim, rtol_ode_sim, type, VVV), options);
end

function cost = costfunction(runningcosts, terminalcosts, system, ...
                    N, T, t0, x0, u, ...
                    atol_ode_sim, rtol_ode_sim, type, xhat11, xhat22)
    cost = 0;
    x = zeros(N+1, length(x0));
    x = computeOpenloopSolution(system, N, T, t0, x0, u, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x(k,:), u(:,k), xhat11(k,:), xhat22(k,:));
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x(N+1,:),xhat11, xhat22);
end

function [c,ceq] = nonlinearconstraints(constraints, ...
    terminalconstraints, system, ...
    N, T, t0, x0, u, atol_ode_sim, rtol_ode_sim, type,VVV)
    x = zeros(N+1, length(x0));
    x = computeOpenloopSolution(system, N, T, t0, x0, u, ...
                                atol_ode_sim, rtol_ode_sim, type);
    c = [];
...
%     ceq = [ceq ceqnew];
    ...
    [cnew, ceqnew] = constraints(t0+T, x(1,:),u(:,1),VVV);
    c = [c cnew];
    ceq = [ceq ceqnew];
    for k=2:N+1
        [cnew, ceqnew] = terminalconstraints(t0+k*T,x(k,:));
    c = [c cnew];
    ceq = [ceq ceqnew];
    end
%     [cnew, ceqnew] = terminalconstraints(t0+(N+1)*T,x(N+1,:));
...
%              plotTrajectories, T, t0, x0, u, ...
%              atol_ode, rtol_ode, type, iprint, exitflag, output, t_Elapsed)
%  
%     printHeader();
%     printClosedloopData( u, x0, t_Elapsed);
%     switch exitflag
%         case -2
