function [ fx ] = LMPC_fmincon_cost( u,X0,U_prev,Hp,Q,Qf,R,R1,P,T)
...
for i=1:1:Hu
    ...
    Ur=[Fur(i,1);Fvr(i,1);Frr(i,1)];
    
%     Ur=[T1(i,1);T2(i,1);T3(i,1);T4(i,1)];
    
    fx=fx+(Ur-U(:,i))'*R*(Ur-U(:,i));
...
% for i=1:1:Hu_detla
%     
%     fx=fx+(U(:,i+1)-U(:,i))'*R1*(U(:,i+1)-U(:,i));
% 
% end

fx=fx+(U(:,1)-U_prev)'*R1*(U(:,1)-U_prev);
