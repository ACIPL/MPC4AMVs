classdef AUV < Model
    ...
    methods
        function obj = AUV(coef,ndof,ctrlmat,X0,U0)
            if nargin == 5
                % Currently 3DOF model is supported and should be extended
                % in the future
                if ndof == 3
                    assert(length(X0) == 6,'For 3 DOF AUV model X = [x;y;psi;u;v;r].');
                    [row,col] = size(ctrlmat);
                    assert(row == 3,'For 3 DOF AUV model tau = [Fu;Fv;Fr].')
                    assert(length(U0) == col,'The thrust input must be consistent with the input matrix B; tau = BU')
                ...
                msg = 'Five input arguments (coef, ndof, ctrlmat, X0, U0) should be provided.';
        ...
        function obj = advance(obj,U,W,dt)
            if obj.Ndof == 3 
                [row,col] = size(obj.CtrlMat);
                assert(length(U) == col,'The thrust input must be consistent with the input matrix B; tau = BU')
                assert(length(W) == row,'The disturbance dimension must be consistent length(W) =length(tau)')
                B = obj.CtrlMat;            
                tau = B*U;
                disturbed_tau = tau + W;
                disturbed_U = pinv(B)*disturbed_tau;
